...
{
	class HourTests : DateTimeFunctionsTestBase
	{
		...
		{
			var func = new Hour();
			var result = func.Execute(FunctionsHelper.CreateArgs(GetTime(9, 13, 14)), this.ParsingContext);
			Assert.AreEqual(9, result.Result);

			result = func.Execute(FunctionsHelper.CreateArgs(GetTime(23, 13, 14)), this.ParsingContext);
