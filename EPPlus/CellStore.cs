...
internal class CellStore<T> : ICellStore<T>, IDisposable// : IEnumerable<ulong>, IEnumerator<ulong>
{
	...
	/// Set Value for Range
	/// </summary>
	/// <param name="fromRow">The row to start inserting at.</param>
	/// <param name="fromColumn">The column to start inserting at.</param>
	/// <param name="toRow">The last row to insert to.</param>
	/// <param name="toColumn">The last columnn to insert to.</param>
	/// <param name="updater">A function for storing the <paramref name="value"/> in the CellStore.</param>
	/// <param name="value">The value[s] to be inserted in the range.</param>
	public void SetRangeValueSpecial(int fromRow, int fromColumn, int toRow, int toColumn, CellStoreDelegates<T>.SetRangeValueDelegate updater, object value)
	{
		lock (this._columnIndex)
		{
			...
			for (int rowIx = fromRow; rowIx <= toRow; rowIx++)
			{
				...
				if (!pages.ContainsKey(pageIx))
					pages.Add(pageIx, new List<int>());
				pages[pageIx].Add(rowIx);
			}

			for (int colIx = fromColumn; colIx <= toColumn; colIx++)
			{
				var col = Array.BinarySearch(this._columnIndex, 0, this.ColumnCount, new IndexBase() { Index = (short)(colIx) });
				...
				foreach (var pair in pages)
				{
					...
					foreach (var rowIx in pair.Value)
					{
						if (col >= 0)
						{
							var pos = this._columnIndex[col].GetPosition(rowIx);
							if (pos < 0)
							{
								pos = ~pos;
								if (pos - 1 < 0 || this._columnIndex[col]._pages[pos - 1].IndexOffset + PageSize - 1 < rowIx)
								{
									this.AddPage(_columnIndex[col], pos, page);
								}
								else
								{
									pos--;
							...
							{
								cellPos = ~cellPos;
								this.AddCell(this._columnIndex[col], pos, cellPos, ix, default(T));
								updater(this._values, pageItem.Rows[cellPos].IndexPointer, rowIx, colIx, value);
							}
							else
							{
						...
						{
							col = ~col;
							this.AddColumn(col, colIx);
							this.AddPage(this._columnIndex[col], 0, page);
							short ix = (short)(rowIx - (page << pageBits));
							this.AddCell(this._columnIndex[col], 0, 0, ix, default(T));
							updater(this._values, this._columnIndex[col]._pages[0].Rows[0].IndexPointer, rowIx, colIx, value);
						}
					}
				}
			}
	...
	/// </summary>
	/// <param name="row">The row whose value should be set.</param>
	/// <param name="column">The column whose value should be set.</param>
	/// <param name="updater">A function that maps a value to a specific location.</param>
	/// <param name="value">The value to be stored by the updater.</param>
	public void SetValueSpecial(int row, int column, CellStoreDelegates<T>.SetValueDelegate updater, object value)
	{
		lock (this._columnIndex)
		{
			var col = Array.BinarySearch(this._columnIndex, 0, ColumnCount, new IndexBase() { Index = (short)(column) });
			var page = (short)(row >> pageBits);
			if (col >= 0)
			{
				//var pos = Array.BinarySearch(_columnIndex[col].Pages, 0, _columnIndex[col].Count, new IndexBase() { Index = page });
				...
				{
					pos = ~pos;
					if (pos - 1 < 0 || this._columnIndex[col]._pages[pos - 1].IndexOffset + PageSize - 1 < row)
					{
						this.AddPage(this._columnIndex[col], pos, page);
					}
					else
				...
				{
					cellPos = ~cellPos;
					this.AddCell(_columnIndex[col], pos, cellPos, ix, default(T));
					updater(this._values, pageItem.Rows[cellPos].IndexPointer, value);
				}
				else
				{
					updater(this._values, pageItem.Rows[cellPos].IndexPointer, value);
			...
			else //Column does not exist
			{
				...
				this.AddCell(this._columnIndex[col], 0, 0, ix, default(T));
				updater(this._values, this._columnIndex[col]._pages[0].Rows[0].IndexPointer, value);
			}
		}
	}
