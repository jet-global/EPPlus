...
namespace OfficeOpenXml.Table.PivotTable.DataCalculation.ShowDataAsCalculation
{
	...
	internal class PercentOfColCalculator : ShowDataAsCalculatorBase
	{
		...
		public PercentOfColCalculator(ExcelPivotTable pivotTable, int dataFieldCollectionIndex, TotalsFunctionHelper totalsCalculator) 
			...
			List<PivotCellBackingData> columnGrandTotalsValuesLists)
		{
			...
			else
			{
				...
				var denominator = rowGrandTotalsValuesLists
			...
			bool isRowTotal)
		{
			...
			if (Convert.ToDouble(grandTotalBackingData.Result) == 0d)
