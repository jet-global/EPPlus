...
namespace OfficeOpenXml.FormulaParsing.Excel.Functions.Math
{
	...
	{
		/// <summary>
		/// Get the hyperbolic sine of an input.
		/// </summary>
		/// <param name="arguments">Input to have its hyperbolic sine calculated.</param>
		/// <param name="context">Unused, this is information about where the function is being executed.</param>
		/// <returns>Returns the hyperbolic sine of a number.</returns>
		public override CompileResult Execute(IEnumerable<FunctionArgument> arguments, ParsingContext context)
		{
			...
			if (!ConvertUtil.TryParseObjectToDecimal(argument, out double result))
				return new CompileResult(eErrorType.Value);
			return this.CreateResult(System.Math.Sinh(result), DataType.Decimal);
		}
	}
}
